...
@pytest.mark.parametrize('metadata, key, expected', [
    ...
    (empty_experiment_description, 'experiment_description', ['does not match \'^(.|\\\\s)*\\\\S(.|\\\\s)*$\'']),
    (empty_session_description, 'session_description', ['does not match \'^(.|\\\\s)*\\\\S(.|\\\\s)*$\'']),
    (empty_session_id, 'session_id', ['does not match \'^(.|\\\\s)*\\\\S(.|\\\\s)*$\'']),
    ...
    (subject_with_empty_values, 'subject1', [
    ...
    (data_acq_device_with_no_values, 'data_acq_device1', [
        'does not match \'^(.|\\\\s)*\\\\S(.|\\\\s)*$\'',
        'does not match \'^(.|\\\\s)*\\\\S(.|\\\\s)*$\'',
        'does not match \'^(.|\\\\s)*\\\\S(.|\\\\s)*$\'',
        'does not match \'^(.|\\\\s)*\\\\S(.|\\\\s)*$\'',
...
def test_metadata_validation_verification_on_invalid_data(metadata, key, expected):
    ...
    for index, error in enumerate(errors):
        assert expected[index] in error, f'Expected error not found - ${expected[index]}'
